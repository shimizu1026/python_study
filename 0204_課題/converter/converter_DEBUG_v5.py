                    ...
                    if len(pd_parts) > 0:
                        ...
                        if len(heavy_pd) > 0:
                            ...
                            for info in heavy_pd:
                                serial = info['serial']
                                parts_num = info['parts']
                                # 該当する行を探す
                                matching_rows = item_rows[(item_rows['SERIAL'] == serial) & (item_rows.iloc[:, 3] == parts_num)]
                                if len(matching_rows) > 0:
                                    first_row = matching_rows.iloc[0]
                                    parts_name = str(first_row.iloc[5])[:50] if len(first_row) > 5 else 'N/A'
                                    supply = str(first_row.iloc[13])[:20] if len(first_row) > 13 else 'N/A'
                                    material = str(first_row.iloc[9])[:20] if len(first_row) > 9 else 'N/A'
                                    print(f"{serial}-{parts_num}: {info['weight']:.2f}kg")
                                    print(f"  NAME={parts_name}")
                                    print(f"  MATERIAL={material}, SUPPLY={supply}")
