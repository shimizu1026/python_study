...
class ExcelConverter:
    ...
    def classify_category(row, all_df):
        ...
        try:
            ...
            # 【判定1】PARENT ASSY NOが空 → 対象外
            if pd.isna(parent_assy) or str(parent_assy).strip() == '':
                return None
            
            # 【判定2】SUPPLY判定（ES, SU, ST, CS）
            if any(x in supply for x in ['ES', 'SU', 'ST', 'CS']):
                # Roll/Rollerが含まれるか
                if 'ROLL' in parts_name:
                    # BRG/Bearingが含まれるか
                    if 'BRG' in parts_name or 'BEARING' in parts_name:
                        return None  # 対象外
                    # Roll/Rollerだけなら次へ
                else:
                    return None  # 対象外
            
            # 【判定3】PARENT ASSY NOの桁数（3-6桁）
            if pd.notna(parent_assy):
                parent_str = str(parent_assy).replace('.0', '').strip()
                if 3 <= len(parent_str) <= 6:
                    return 'De'
            ...
            if not is_common_steel:
                # 【判定5-2】一般鋼材以外で 400kg以上
                if total_weight >= 400:
                    return 'Ds'
            else:
                # 【判定5-3】構成部材にSPCC/SPHC/SS以外で300kg以上のものがあるか
                # 同じSERIALの他の部品を確認
                if pd.notna(serial):
            ...
            try:
                ...
                for serial, parts_list in serial_groups.items():
                    for parts_group in parts_list:
                        try:
                            # 代表行で判定
                            category = self.classify_category(parts_group['representative_row'], self.input_data)
                            ...
                            category_stats[category]['weight'] += parts_group['weight']
                            category_stats[category]['parts'].append({
                                'weight': parts_group['weight'],
